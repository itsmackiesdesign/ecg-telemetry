...
export async function POST(request: Request) {
  ...
  const user = await getAppUser(); if (!user || user.role !== "doctor") return reply({ error: "sign_in_required" }, 401);
  ...
  try {
    ...
    try {
      await env.DB.prepare("INSERT INTO center_handovers (id, center_id, clinician_user_id, clinician_email, patient_json, ecg_object_key, ecg_content_type, ecg_bytes, status) VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'new')")
        .bind(id, context.data.centerId, user.id, user.email, JSON.stringify({ ...context.data, patientId: context.data.patientId || null }), objectKey, mime, bytes.byteLength).run();
