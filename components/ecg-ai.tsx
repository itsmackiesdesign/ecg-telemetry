...
export function AiControls({ai,lang,onReview}:{ai:ReturnType<typeof useEcgAnalysis>;lang:Language;onReview:()=>void}){
 ...
 <p className="ai-privacy">{tr('Up to 5 MB for GPT analysis. No automatic upload. Patient ID is excluded. The app does not save the image or response on its server; OpenAI retention rules still apply.','GPT таҳлили учун 5 МБгача. Автоматик юклаш йўқ. Бемор IDси юборилмайди. Илова серверда тасвир ва жавобни сақламайди; OpenAI сақлаш қоидалари амал қилади.','Для GPT-анализа — до 5 МБ. Автоматической отправки нет. ID пациента не передаётся. Приложение не сохраняет изображение и ответ на сервере; действуют правила хранения OpenAI.')}</p>
