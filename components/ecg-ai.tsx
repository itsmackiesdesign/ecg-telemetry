...
export function useEcgAnalysis(file:File|null,patient:ClinicalContext['patient'],lang:Language){
 ...
 async function analyze(){
  ...
  try{
   ...
   const response=await fetch('/api/ecg/analyze',{method:'POST',body:form,signal:abort.signal,cache:'no-store'});
...
export function AiControls({ai,lang,onReview}:{ai:ReturnType<typeof useEcgAnalysis>;lang:Language;onReview:()=>void}){
 ...
 const errors:Record<string,string>={
  ...
  file_too_large:tr('The image must be smaller than 15 MB.','Тасвир 15 МБдан кичик бўлиши керак.','Изображение должно быть меньше 15 МБ.'),
 ...
 <p className="ai-privacy">{tr('No automatic upload. Patient ID is excluded. The app does not save the image or response on its server; OpenAI retention rules still apply.','Автоматик юклаш йўқ. Бемор IDси юборилмайди. Илова серверда тасвир ва жавобни сақламайди; OpenAI сақлаш қоидалари амал қилади.','Автоматической отправки нет. ID пациента не передаётся. Приложение не сохраняет изображение и ответ на сервере; действуют правила хранения OpenAI.')}</p>
...
export function AiResult({result,lang}:{result:AnalysisEnvelope;lang:Language}){
