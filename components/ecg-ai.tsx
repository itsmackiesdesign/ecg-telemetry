...
export function AiControls({ai,lang,onReview}:{ai:ReturnType<typeof useEcgAnalysis>;lang:Language;onReview:()=>void}){
 ...
 const errors:Record<string,string>={
  ...
  offline:tr('AI analysis requires internet access. Offline clinician assessment remains available.','СИ таҳлили учун интернет керак. Шифокор баҳолаши офлайн мавжуд.','Для ИИ-анализа нужен интернет. Оценка врачом доступна офлайн.'),
 ...
 <p className="ai-privacy">{tr('Up to 5 MB for AI analysis. No automatic upload. Patient ID is excluded. The image and validated result are saved to your case history; the external analysis service retention rules still apply.','СИ таҳлили учун 5 МБгача. Автоматик юклаш йўқ. Бемор IDси юборилмайди. Тасвир ва тасдиқланган жавоб ҳолатлар тарихида сақланади; ташқи таҳлил хизмати сақлаш қоидалари амал қилади.','Для ИИ-анализа — до 5 МБ. Автоматической отправки нет. ID пациента не передаётся. Изображение и проверенный ответ сохраняются в вашей истории; действуют правила хранения внешнего сервиса анализа.')}</p>
...
export function AiResult({result,lang,reveal=false}:{result:AnalysisEnvelope;lang:Language;reveal?:boolean}){
 ...
 return <div className="ai-result"><CoronaryState analysis={a} lang={lang} reveal={animateReveal}/>{(a.coronary_state===undefined||a.review_priority==='emergency_review')&&<div className={'risk-box '+(a.review_priority==='emergency_review'?'high':a.review_priority==='urgent_review'?'medium':'neutral')}><CircleAlert/><div><small>AI · {tr('NOT CONFIRMED','ТАСДИҚЛАНМАГАН','НЕ ПОДТВЕРЖДЕНО')}</small><h3>{title}</h3><p>{a.priority_reason}</p></div></div>}<div className="ai-quality"><ShieldCheck size={17}/><span>{tr('Recording quality','Ёзув сифати','Качество записи')}: <b>{quality}</b></span></div><p className="ai-summary">{a.summary_for_clinician}</p>
 ...
 <div className="info-box"><ShieldCheck size={18}/><p>{tr('ACS is not excluded. AI findings require physician confirmation. Review the original ECG and clinical context.','ЎКС истисно қилинмайди. СИ хулосаси шифокор тасдиғини талаб қилади. Асл ЭКГ ва клиник ҳолатни текширинг.','ОКС не исключён. Выводы ИИ требуют подтверждения врачом. Проверьте исходную ЭКГ и клиническую картину.')}</p></div>
 ...
 <p className="ai-meta">{new Date(result.analyzed_at).toLocaleString(lang==='uz'?'uz-Cyrl':lang)}</p></div>
}
