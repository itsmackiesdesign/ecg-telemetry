...
export function SaveAssessment({patient,finding,ai,file,lang}:{patient:Record<string,unknown>;finding:string;ai:any;file:File|null;lang:string}){
 ...
 return <div className="inline-actions"><Button onClick={save} disabled={busy}>{busy?'…':lang==='ru'?'Сохранить оценку':lang==='uz'?'Баҳолашни сақлаш':'Save assessment'}</Button><span role="status">{message|| (ai?.assessment_id ? (lang==='ru'?'GPT-анализ сохранён автоматически':'GPT analysis saved automatically'):'')}</span></div>
}
